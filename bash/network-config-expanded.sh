...
interfacestore=$(lshw -class network|awk '/logical name:/{print $3}')
for k in $interfacestore;do
  if [[ $k = lo* ]] ; then continue ; fi
  ipv4_address=$(ip a s $k|awk -F '[/ ]+' '/inet /{print $3}')
  ipv4_hostname=$(getent hosts $ipv4_address|awk '{print $2}')
  network_address=$(ip route list dev $k scope link|cut -d ' ' -f 1)
